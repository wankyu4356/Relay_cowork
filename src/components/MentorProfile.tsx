...
export function MentorProfile({ onBack, onBook, mentor, networkDistance, connectionPath, onNavigate }: MentorProfileProps) {
  ...
  const getBadgeStyle = (badge: string) => {
    switch (badge) {
      case 'platinum': return 'bg-iris-600 text-white';
      ...
      <div className="container-web py-8">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-[1fr,360px] gap-6">
          ...
          <div className="space-y-6">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-6">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-2xl font-semibold tracking-tight text-zinc-900 tnum">{mentor.rating}</span>
                        <span className="text-zinc-600">({mentor.reviews}개 리뷰)</span>
                      </div>
            ...
            {networkDistance && (
              <Card className="p-4 bg-iris-50 border-iris-100 mb-4">
            ...
            {!networkDistance && (
              <Card className="p-4 bg-zinc-50 border-zinc-200/80 mb-4">
                <div className="flex items-start gap-3">
                  <Users className="w-6 h-6 text-zinc-400 flex-shrink-0" />
                  <div className="flex-1">
              ...
              <Tabs value={selectedTab} onValueChange={setSelectedTab}>
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        <div key={index} className="flex items-center gap-3">
                          <CheckCircle className="w-5 h-5 text-iris-600 flex-shrink-0" />
                ...
                <TabsContent value="experience" className="space-y-6">
                  <div>
                    ...
                    <div className="space-y-3">
                      {mockTimeline.map((item, index) => (
                        ...
                        >
                          <div className="w-1 bg-gradient-to-b from-iris-400 to-iris-600 rounded-full flex-shrink-0" />
                          <div className="flex-1 pb-6">
                            <div className="text-sm text-zinc-600 mb-1">{item.period}</div>
                            <div className="font-semibold mb-1">{item.title}</div>
                  ...
                  <div>
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      ...
                      <Card className="p-4 text-center bg-iris-50 border-iris-100">
                        <div className="text-3xl font-semibold tracking-tight text-iris-600 mb-1 tnum">35건</div>
                        <div className="text-sm text-zinc-600">총 세션 수</div>
                      </Card>
                      <Card className="p-4 text-center bg-iris-50 border-iris-100">
                        <div className="text-3xl font-semibold tracking-tight text-iris-600 mb-1 tnum">4.9</div>
                ...
                <TabsContent value="reviews" className="space-y-4">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        <span className="text-4xl font-semibold tracking-tight text-zinc-900 tnum">{mentor.rating}</span>
                        <div>
                          <div className="flex items-center gap-1">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <Star 
                                key={star}
                                className={`w-5 h-5 ${star <= Math.floor(mentor.rating) ? 'fill-amber-400 text-amber-400' : 'text-zinc-300'}`}
                              />
                            ))}
                          </div>
                          <div className="text-sm text-zinc-600">{mentor.reviews}개의 리뷰</div>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  {reviewsLoading && (
                    <div className="flex items-center justify-center py-8">
                      <Loader2 className="w-6 h-6 animate-spin text-iris-500 mr-2" />
                      <span className="text-zinc-400">리뷰를 불러오는 중...</span>
                    </div>
                  )}
                  ...
                  {!reviewsLoading && reviews.map((review, index) => (
                    ...
                    >
                      <Card className="p-5 bg-zinc-50">
                        ...
                        <p className="text-zinc-700 mb-3 leading-relaxed">{review.content}</p>
                ...
                <TabsContent value="schedule" className="space-y-4">
                  <Card className="p-4 bg-iris-50 border-iris-100">
                    <div className="flex gap-3">
                      <Calendar className="w-6 h-6 text-iris-600 flex-shrink-0" />
                      <div>
                            ...
                            return (
                              <Button
                                ...
                                className={isSelected
                                  ...
                                  : 'hover:bg-iris-50 hover:border-iris-400'}
          ...
          <div className="space-y-4">
            ...
            <Card className="p-6 sticky top-24">
              <div className="text-center mb-6">
                <div className="text-sm text-zinc-600 mb-2">60분 세션</div>
                <div className="text-4xl font-semibold tracking-tight text-zinc-900 mb-1 tnum">
                  {mentor.price.toLocaleString()}원
                </div>
                <div className="text-sm text-zinc-400">VAT 포함</div>
              </div>
              ...
              <div className="space-y-3 mb-6 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-zinc-600">평균 응답</span>
                  <span className="font-medium">{mentor.responseTime}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-zinc-600">총 세션</span>
                  <span className="font-medium">{mentor.sessions}건</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-zinc-600">합격률</span>
                  <span className="font-medium text-iris-600">{mentor.successRate}%</span>
                </div>
              </div>
              ...
              <div className="mt-6 p-4 bg-iris-50 border border-iris-100 rounded-lg">
                <div className="flex gap-2 text-sm">
                  <Award className="w-5 h-5 text-iris-600 flex-shrink-0" />
